...
export default function Overview() {
  ...
  const activity = useMemo(() => {
    const items = [];
    bets.forEach((b) => items.push({ id: "b" + b.id, user: b.userEmail, action: `Đặt cược ${b.hallName} · ${b.amount} coin`, time: b.created_date, tone: b.result === "win" ? "green" : b.result === "loss" ? "red" : "amber", tag: b.result }));
    txs.forEach((t) => items.push({ id: "t" + t.id, user: t.userEmail, action: `${t.type === "deposit" ? "Nạp" : "Rút"} ${t.amount} coin · ${t.status}`, time: t.created_date, tone: t.status === "completed" ? "green" : t.status === "rejected" ? "red" : "amber", tag: t.status }));
