      ...
      if (tx.userId) {
        ...
        pushNotification(tx.userId, {
          ...
          body: status === "completed"
            ? `Admin đã duyệt đơn rút ${tx.amount.toLocaleString()} coin. Tiền sẽ về tài khoản ngân hàng trong ít phút.`
            : `Đơn rút ${tx.amount.toLocaleString()} coin bị từ chối. Vui lòng liên hệ hỗ trợ để biết chi tiết.`,
        });
      }
  ...
  return (
    <div className="space-y-4">
            ...
            {rows.length === 0 ? (
              ...
              rows.map((t) => (
                <tr key={t.id} className="border-t border-white/5 hover:bg-white/[0.02]">
                  ...
                  <Td className="font-bold text-[#bd9c59]">{t.amount?.toLocaleString()} coin</Td>
      ...
      <ConfirmDialog
        ...
        desc={`${confirm?.tx?.userEmail} · ${confirm?.tx?.amount?.toLocaleString()} coin · ${confirm?.tx?.method}`}
