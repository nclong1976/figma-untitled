...
export default function BottomNav() {
  ...
  const isActive = (item) => item.match.some((m) => (m === "/dashboard" ? p === "/dashboard" : p.startsWith(m)));
  ...
  return (
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md z-50 bg-[#0A0E1A]/95 backdrop-blur-xl border-t border-white/10 shadow-[0_-4px_20px_rgba(0,0,0,0.4)] pb-safe">
      <div className="grid grid-cols-4 h-[62px] items-center px-1">
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className="relative flex flex-col items-center justify-center py-1.5 focus:outline-none transition-all active:scale-95 rounded-xl hover:bg-white/5"
            >
              {active && (
                <span className="absolute top-0 w-8 h-[3px] bg-[#FFD700] rounded-full shadow-[0_0_8px_#FFD700]" />
              )}
              <Icon
              ...
              />
              <span
                className={`text-[11px] font-medium leading-tight mt-1 transition-colors ${
