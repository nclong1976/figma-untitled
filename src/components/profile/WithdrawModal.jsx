...
export default function WithdrawModal({ open, onOpenChange, balance, minTurnover, turnover, linked, onSubmit, withdrawRequests = [] }) {
  ...
  const handleSubmit = () => {
    ...
    notifyAdmins({
      ...
      body: `${user?.account || "Người dùng"} yêu cầu rút ${numAmount.toLocaleString()} coin → ${selected.bankName} · ${selected.accountNumber}`,
      requestId: reqId,
      userId: user?.id,
                ...
                <>
                  ...
                  <div className="bg-[#2a2040] rounded-xl p-3 space-y-2">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span>Số dư: <strong className="text-[#bd9c59]">{balance.toLocaleString()} coin</strong></span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                  ...
                  {/* Số tiền rút */}
                  <div>
                    <Label className="text-xs text-white/70 mb-2 block">Số tiền rút (coin)</Label>
                    <Input
                      type="number"
                      min={1}
                      max={balance}
            ...
            <div className="space-y-3">
              {withdrawRequests.length === 0 ? (
                <div className="text-center py-10 text-white/40">
                  ...
                  <p className="text-sm">Chưa có đơn rút tiền nào</p>
                </div>
              ) : (
                    ...
                    <div key={req.id} className="bg-[#2a2040] rounded-xl p-3.5 border border-white/5">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex-1">
                          <p className="text-sm font-semibold text-white">{req.amount?.toLocaleString()} coin</p>
