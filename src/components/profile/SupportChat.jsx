      ...
      <SheetContent side="bottom" className="bg-[#1e1832] border-[#323b51] text-white rounded-t-2xl p-0 h-[75vh] flex flex-col">
        ...
        {!user ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-3 p-6 text-center">
            ...
            <Button className="bg-[#bd9c59] text-[#1e1832] hover:bg-[#cfb06a]" onClick={() => { onOpenChange?.(false); }}>Đăng nhập</Button>
          </div>
        ) : (
          <>
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
              {messages.length === 0 && <p className="text-center text-white/40 text-sm py-8">Chào bạn, đội ngũ CSKH 24/7 sẵn sàng hỗ trợ. Bạn cần giúp gì ạ?</p>}
              {messages.map((m) => (
            ...
            <div className="flex items-center gap-2 px-3 py-3 border-t border-[#323b51]">
              ...
              <Input value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendText()} placeholder="Nhập tin nhắn..." className="bg-[#2a2040] border-[#3a2d52] text-white placeholder:text-white/50" />
